...
/***/ function(module, exports, __webpack_require__) {
	...
	function syncBoard() {
	  ...
	  if (freeSpacesChanged()) {
	    gameRenderer.clearAllTileClasses(target).addRandomTile().clearAllValues(target).addTilesTo(target).printScoreTo(target);
	  } else {
	    gameRenderer.clearAllTileClasses(target).clearAllValues(target).addTilesTo(target);
	  }
	}
